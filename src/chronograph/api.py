    ...
    def app(environ, start_response):
        ...
        if method == "POST" and path == "/sources":
            ...
            if not required <= set(payload):
                return _json_response(start_response, "400 Bad Request", {"error": "source_id, source_type, and text are required"})
            items = chronograph.ingest(
                payload["source_id"], payload["source_type"], payload["text"],
                timestamp=payload.get("timestamp"), author=payload.get("author"), metadata=payload.get("metadata") or {},
            )
            return _json_response(start_response, "201 Created", {"work_items": [item.to_dict() for item in items]})
        if method == "GET" and path == "/work-items":
            return _json_response(start_response, "200 OK", [item.to_dict() for item in chronograph.work_items()])
        if method == "GET" and path == "/active":
            return _json_response(start_response, "200 OK", [item.to_dict() for item in chronograph.active_work_items()])
        if method == "GET" and path == "/review":
            return _json_response(start_response, "200 OK", [item.to_dict() for item in chronograph.review_items()])
